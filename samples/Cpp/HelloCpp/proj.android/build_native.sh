...
function copy_jpushfile()
{
	for file in "$APP_ANDROID_ROOT"/libs/libsys.so
    do
    if [ -d "$file" ]; then
        cp -rf "$file" "$APP_ANDROID_ROOT"/libs/armeabi/libsys.so
    fi
    
    if [ -f "$file" ]; then
        cp "$file" "$APP_ANDROID_ROOT"/libs/armeabi/libsys.so
    fi
    done
}
